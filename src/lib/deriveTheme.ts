...
export function deriveFlowTheme(
  ...
  const headerBg = c.headerBg ?? (extHeader && !isNearWhite(extHeader) && !isExtHeaderGenericBlue ? extHeader : null) ?? g.headerBg ?? DEFAULT.headerBg;
  ...
  const footerBgFinal = c.footerBg ?? (extFooter && !isGenericBlue && !isFrameworkYellow ? extFooter : null) ?? g.footerBg ?? null;
